...
const API_GATEWAY_URL = "https://esc6t1blhc.execute-api.ap-south-1.amazonaws.com/default/uploadVideo";
...
const videoObjectLinkContainer = document.getElementById("video-object-link-container");
...
// --- EVENT LISTENERS ---
copyFolderLinkButton.addEventListener('click', () => copyLinkToClipboard(s3FolderLink.href, copyFolderLinkButton));
copyVideoLinkButton.addEventListener('click', () => copyLinkToClipboard(s3ObjectLink.href, copyVideoLinkButton));
...
uploadButton.addEventListener("click", async () => {
    ...
    if (allFilesToUpload.length === 0) return alert("Please select at least a video or a zip file to upload.");
    ...
    try {
        ...
        if (videoFile) {
            ...
            objectKey = `${finalS3Folder}/Game Video/${sanitizedGameName}.${extension}`;
        }
        
        // <<< IMPORTANT >>>: Replace these values with your actual bucket name and region.
        const BUCKET_NAME = "full-game-footage";
...
const displayS3Link = (bucket, folder, region, objectKey = null) => {
    if (!bucket || !folder || !region || bucket.includes("your-unique-bucket")) {
        console.warn("Could not display S3 link. Please configure bucket name and region in script.js");
        return;
    }
    const folderUrl = `https://s3.console.aws.amazon.com/s3/buckets/${bucket}?region=${region}&prefix=${folder}/`;
...
gameNameInput.addEventListener('input', () => {
    ...
    const gameName = gameNameInput.value.trim();
    const sanitizedGameName = gameName.replace(/\s+/g, '-').replace(/[^a-zA-Z0-9-]/g, '');
    folderNameInput.value = gameName ? `${dateString}-${sanitizedGameName}` : `${dateString}-your-game-name`;
});
...
function populateAndShowModal() {
    ...
    for (const teamName in playersByTeam) {
        ...
        playersByTeam[teamName].forEach(player => {
            ...
            row.innerHTML = `<div class="player-info"><span class="player-name">${playerName}</span></div><div class="package-options"><select><option value="none" ${existingPackage === 'none' ? 'selected' : ''}>None</option><option value="stat" ${existingPackage === 'stat' ? 'selected' : ''}>Stat Package</option><option value="highlight" ${existingPackage === 'highlight' ? 'selected' : ''}>Highlight Package</option><option value="both" ${existingPackage === 'both' ? 'selected' : ''}>Both Packages</option></select></div>`;
